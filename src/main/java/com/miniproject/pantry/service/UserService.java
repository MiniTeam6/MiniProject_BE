...
import com.miniproject.pantry.core.exception.Exception401;
import com.miniproject.pantry.core.exception.Exception404;
import com.miniproject.pantry.core.exception.Exception500;
import com.miniproject.pantry.dto.user.UserResponse.LoginOutDTO;
import com.miniproject.pantry.dto.user.UserResponse.SignupOutDTO;
import com.miniproject.pantry.dto.user.UserResponse.StatusUpdateOutDTO;
import com.miniproject.pantry.dto.user.UserResponse.UserApprovalListOutDTO;
import com.miniproject.pantry.dto.user.UserResponse.UserDetailOutDTO;
import com.miniproject.pantry.dto.user.UserResponse.UserListOutDTO;
import com.miniproject.pantry.dto.user.UserResponse.UserRoleUpdateOutDTO;
...
public class UserService {
    ...
    @SentrySpan
    @Transactional
    public SignupOutDTO 회원가입(UserRequest.SignupInDTO signupInDTO, MultipartFile image) throws IOException {
    ...
    public Object[] 로그인(UserRequest.LoginInDTO loginInDTO) {
        ...
        try {
            ...
            result[0] = new LoginOutDTO(myUserDetails.getUser());
    ...
    @SentrySpan
    public UserDetailOutDTO 회원상세보기(Long id) {

        User userPS = userRepository.findById(id).orElseThrow(
                ()-> new Exception400("id", "해당 유저를 찾을 수 없습니다")
        );
        return new UserDetailOutDTO(userPS);
    }

    @SentrySpan
    @Transactional
    public UserDetailOutDTO 회원정보수정(Long id, UserRequest.ModifyInDTO modifyInDTO, MultipartFile image) throws IOException {
        User userPS = userRepository.findById(id).orElseThrow(
                ()-> new Exception400("id", "해당 유저를 찾을 수 없습니다")
        );
    ...
    @SentrySpan
    public Page<UserListOutDTO> 회원리스트검색(String type, String keyword, int page, int size){
        ...
        if(type.equals("username")){
            ...
            return users.map(request-> new UserListOutDTO(request));
        }else{
    ...
    @SentrySpan
    @Transactional
    public Page<UserApprovalListOutDTO> 회원전체리스트(int page, int size){
        Pageable pageable = PageRequest.of(page, size,Sort.by(Sort.Direction.ASC, "username"));
        Page<User> users = userRepository.findUsersByStatus(true,pageable);
        return users.map(request-> new UserApprovalListOutDTO(request));
    }

    @SentrySpan
    @Transactional
    public Page<UserApprovalListOutDTO> 회원전체리스트가입일정렬(int page, int size){
    ...
    @SentrySpan
    @Transactional
    public List<UserListOutDTO> 회원전체리스트2(){
        List<User> users = userRepository.findUsersByStatus2(true);
        List<UserListOutDTO> userDTOs = users.stream()
                .map(user -> new UserListOutDTO(user))
                .collect(Collectors.toList());
        return userDTOs;
    }
    ...
    @SentrySpan
    @Transactional
    public UserRoleUpdateOutDTO 권한업데이트(UserRequest.RoleUpdateInDTO roleUpdateInDTO){
        ...
        try{
            ...
            return new UserRoleUpdateOutDTO(userPS);
        }catch (Exception e){
    ...
    @SentrySpan
    @Transactional
    public StatusUpdateOutDTO 회원가입승인(UserRequest.StatusUpdateInDTO statusUpdateInDTO){
        ...
        try{
            User userPS=userRepository.save(user.get());
            return new StatusUpdateOutDTO(userPS);
        }catch (Exception e){
    ...
    @SentrySpan
    public Page<UserListOutDTO> 회원가입요청목록(int page, int size){
        Pageable pageable = PageRequest.of(page, size,Sort.by(Sort.Direction.ASC, "username"));
